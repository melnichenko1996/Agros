    ...
    if(isset($_POST["btn_submit_register"]) && !empty($_POST["btn_submit_register"])){
        ...
        if(isset($_POST["captcha"]) && !empty($captcha)){
            ...
            if(($_SESSION["rand"] != $captcha) && ($_SESSION["rand"] != "")){
                ...
                $error_message = "<p class='mesage_error'><strong>Помилка!</strong> Ви ввели невірний код</p>";
            ...
            if(isset($_POST["first_name"])){
                ...
                // Сохраняем в сессию сообщение об ошибке. 
                $_SESSION["error_messages"] .= "<p class='mesage_error'>Відсутнє поле с іменем</p>";
            ...
            if(isset($_POST["last_name"])){
                ...
                // Сохраняем в сессию сообщение об ошибке. 
                $_SESSION["error_messages"] .= "<p class='mesage_error' >Відсутнє поле с прізвищем</p>";
            ...
            if(isset($_POST["email"])){
                ...
                if(!empty($email)){
                    ...
                    if( !preg_match($reg_email, $email)){
                        // Сохраняем в сессию сообщение об ошибке. 
                        $_SESSION["error_messages"] .= "<p class='mesage_error' >>Ви ввели невірний email</p>";
                    ...
                    if($result_query->num_rows == 1){
                        ...
                        if(($row = $result_query->fetch_assoc()) != false){
                                ...
                                // Сохраняем в сессию сообщение об ошибке. 
                                $_SESSION["error_messages"] .= "<p class='mesage_error' >Користувач з такою email адресою вже зареєстрований</p>";
                            ...
                            // Сохраняем в сессию сообщение об ошибке. 
                            $_SESSION["error_messages"] .= "<p class='mesage_error' >Помилка при запиті до БД</p>";
                    ...
                    // Сохраняем в сессию сообщение об ошибке. 
                    $_SESSION["error_messages"] .= "<p class='mesage_error' >Поле для введення Email пусте</p>";
                ...
                // Сохраняем в сессию сообщение об ошибке. 
                $_SESSION["error_messages"] .= "<p class='mesage_error' >Поле для ввода почтового Email не повинно бути порожнім</p>";
            ...
            if(isset($_POST["password"])){
                ...
                // Сохраняем в сессию сообщение об ошибке. 
                $_SESSION["error_messages"] .= "<p class='mesage_error' >Поле для введення пароля пусте</p>";
            ...
            if(!$result_query_insert){
                ...
                $_SESSION["success_messages"] = "<p style=\"color: green; font-size: 16px; text-shadow: 0 1px 3px rgba(0,0,0,.5);\">Реєстрація пройшла успішно!!! <br>Тепер Ви можете авторизуватися використовуючи Ваш логін і пароль.</p>";
            ...
            exit("<p><strong>Помилка!</strong> Відсутній код перевірки, тобто код картинки. Ви можете перейти на <a href=".$address_site."> головну сторінку </a>.</p>");
        }

    }else{

        exit("<p><strong>Помилка!</strong> Ви зайшли на цю сторінку без авторизації, тому у вас немає доступу. Ви можете перейти на <a href=".$address_site."> головну сторінку </a>.</p>");
    }
